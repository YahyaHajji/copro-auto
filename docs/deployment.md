...
4. Dans Vercel, fournir l'URL Neon avec pool de connexions comme `DATABASE_URL`, puis `LICENSE_SIGNING_PRIVATE_KEY`, `LICENSE_KEY_PEPPER`, `LEASE_DAYS=1`, `ADMIN_PASSWORD_HASH` et `ADMIN_SESSION_SECRET` pour Production et Preview. Le desktop renouvelle ce lease de 24 heures au démarrage, toutes les 15 minutes et avant chaque action productive; après son expiration, la grâce locale de 2 jours est strictement en lecture seule. Générer le hash du mot de passe avec `copro-admin-password`; conserver le mot de passe dans un gestionnaire de mots de passe et ne jamais l'ajouter au dépôt.
